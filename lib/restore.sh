...
#
# term_* comes from whichever terminal adapter was loaded; the backticks in
...
pane_command() {   # $1 provider, $2 sessionId, $3 cwd, $4 recorded command, $5 replay policy
    ...
    case $policy in
        ...
        auto)
            if printf '%s' "$cmd" | grep -Eq "$PANEFUL_REPLAY_ALLOW"; then
                printf 'run\t%s' "$cmd"
            else
...
snapshot_panes() {   # $1 snapshot json, $2 replay policy
    ...
    while IFS=$PANEFUL_US read -r wi ti si cwd provider sid name cmd; do
        [ -n "$wi" ] || continue
        out=$(pane_command "$provider" "$sid" "$cwd" "$cmd" "$2")
        mode=${out%%$'\t'*}
        printf '%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s\n' \
            "$wi" "$PANEFUL_US" "$ti" "$PANEFUL_US" "$si" "$PANEFUL_US" "$cwd" "$PANEFUL_US" "$provider" "$PANEFUL_US" \
            "$sid" "$PANEFUL_US" "$name" "$PANEFUL_US" "$mode" "$PANEFUL_US" "${out#*$'\t'}"
...
live_pane_at() {   # $1 wi, $2 ti, $3 si, $4 layout -> id\tcwd\ttitle
    awk -F'\t' -v w="$1" -v t="$2" -v i="$3" -v us="$PANEFUL_US" \
...
evaluate_inplace() {   # $1 snapshot, $2 layout, $3 sessions, $4 start epoch, $5 relaunch, $6 policy
    ...
    while IFS=$PANEFUL_US read -r wi ti si cwd provider sid name mode cmd; do
        [ "$mode" = none ] && continue
        [ -n "$name" ] || name=${cmd%% *}
        paneId=""; liveCwd=""; title=""
        IFS=$PANEFUL_US read -r paneId liveCwd title <<<"$(live_pane_at "$wi" "$ti" "$si" "$2")"
        pid=""
        ...
        elif [ "$5" -eq 0 ] && printf '%s' "$title" | grep -qE "$PANEFUL_AGENT_TITLE_RE"; then
        ...
        printf '%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s\n' \
            "$wi" "$PANEFUL_US" "$ti" "$PANEFUL_US" "$si" "$PANEFUL_US" "$cwd" "$PANEFUL_US" "$name" "$PANEFUL_US" "$mode" "$PANEFUL_US" \
            "$cmd" "$PANEFUL_US" "$paneId" "$PANEFUL_US" "$liveCwd" "$PANEFUL_US" "$title" "$PANEFUL_US" "$decision" "$PANEFUL_US" "$reason"
...
describe_decisions() {   # $1 evaluation TSV, $2 "would" or ""
    ...
    while IFS=$PANEFUL_US read -r wi ti si cwd name mode cmd paneId liveCwd title decision reason; do
        [ -n "$wi" ] || continue
        case $decision in
            type) verb="type" ;;
...
describe_live_panes() {   # $1 snapshot, $2 layout, $3 sessions, $4 relaunch, $5 policy
    ...
    while IFS=$PANEFUL_US read -r _ wi ti paneId cwd title; do
        key="$wi,$ti"
        if [ "$key" != "$prev" ]; then n=0; prev=$key; fi
        n=$((n + 1)); si=$n
        IFS=$PANEFUL_US read -r scwd provider sid name cmd <<<"$(jq -r --argjson wi "$wi" --argjson ti "$ti" --argjson si "$si" "
...
print_rebuild_plan() {   # $1 snapshot, $2 sessions, $3 relaunch, $4 layout style, $5 policy
    ...
    while IFS=$PANEFUL_US read -r w t idx tname pinned n; do
        printf '  tab %s [%s]%s\n' "$idx" "$tname" "$([ "$pinned" = true ] && printf ' (pinned title restored)')"
        for (( k = 0; k < n; k++ )); do
            IFS=$PANEFUL_US read -r cwd provider sid name cmd <<<"$(jq -r ".windows[$w].tabs[$t].terminals[$k]
...
execute_inplace() {   # $1 evaluation, $2 start epoch; prints leftovers as cwd\tprovider\tsid\tname\tcmd
    ...
    while IFS=$PANEFUL_US read -r wi ti si cwd name mode cmd paneId liveCwd title decision reason; do
        ...
        case $decision in
            newtab)
                printf '%s%s%s%s%s\n' "$cwd" "$PANEFUL_US" "$cmd" "$PANEFUL_US" "$name" ;;
            type|prefill)
                wait_ready "$paneId" "$cwd" ||
                    say "tab $ti pane $si: no prompt after ${PANEFUL_READY_TIMEOUT}s; sending anyway"
                if [ "$decision" = type ]; then
                    mkdir "$(claimed_dir "$2")/$(printf '%s' "$cmd" | shasum | cut -c1-16)" 2>/dev/null || true
                fi
                term_type "$paneId" "$cmd" "$([ "$decision" = type ] && echo 1 || echo 0)"
                n=$((n + 1)) ;;
        esac
    done <<<"$1"
...
build_plan() {   # $1 snapshot, $2 sessions, $3 layout style, $4 policy, $5 relaunch
    ...
    for (( w = 0; w < nwin; w++ )); do
        ...
        for (( t = 0; t < ntab; t++ )); do
            IFS=$PANEFUL_US read -r idx tname pinned selected n <<<"$(jq -r ".windows[$w].tabs[$t]
            ...
            for (( k = 0; k < n; k++ )); do
                ...
                if [ "$first" = 1 ]; then
                    printf '%s%s%s%s%s%s%s\n' "$([ "$t" -eq 0 ] && echo W || echo T)" \
                        "$PANEFUL_US" "$cwd" "$PANEFUL_US" "$run" "$PANEFUL_US" "$prefill"
                    first=0
                elif [ "$3" = grid ] && (( k % 2 == 0 )); then
                    printf 'P%sdown%s%s%s%s%s%s%s%s\n' "$PANEFUL_US" "$PANEFUL_US" "$((k - 2))" \
                        "$PANEFUL_US" "$cwd" "$PANEFUL_US" "$run" "$PANEFUL_US" "$prefill"
                else
                    printf 'P%sright%s%s%s%s%s%s%s%s\n' "$PANEFUL_US" "$PANEFUL_US" "$((k - 1))" \
                        "$PANEFUL_US" "$cwd" "$PANEFUL_US" "$run" "$PANEFUL_US" "$prefill"
                fi
            done
...
restore_rebuild() {   # $1 snapshot, $2 sessions, $3 layout style, $4 policy, $5 relaunch
    ...
    created=$(term_build <<<"$plan") || { say "the terminal refused to build the layout"; return 1; }
    nwin=$(grep -c "^W$PANEFUL_US" <<<"$plan" || true)
    ...
    if ! terminal_has env; then
        while IFS=$PANEFUL_US read -r paneId cwd run prefill; do
            [ -n "$paneId" ] || continue
            [ -n "$run" ] && { wait_ready "$paneId" "$cwd" || true; term_type "$paneId" "$run" 1; }
            [ -n "$prefill" ] && { wait_ready "$paneId" "$cwd" || true; term_type "$paneId" "$prefill" 0; }
        done <<<"$created"
    fi
...
restore_unplaced() {   # $1 snapshot, $2 sessions, $3 extra lines: cwd\tcmd\tname
    ...
    local resolved=""
    while IFS=$PANEFUL_US read -r cwd provider sid name; do
        [ -n "$cwd" ] || continue
        session_is_running "$sid" "$2" && { say "$name: already running; no extra pane"; continue; }
        out=$(pane_command "$provider" "$sid" "$cwd" "" run)
        mode=${out%%$'\t'*}
        [ "$mode" = none ] && continue
        resolved+="$cwd$PANEFUL_US${out#*$'\t'}$PANEFUL_US$name"$'\n'
    done <<<"$list"
    [ -n "${3:-}" ] && resolved+="$3"$'\n'
    resolved=$(sed '/^$/d' <<<"$resolved")
    [ -n "$resolved" ] || return 0

    while IFS=$PANEFUL_US read -r cwd cmd name; do
        [ -n "$cwd" ] || continue
        say "${name:-that command} -> its own pane in a 'paneful' tab, $(short "$cwd"): $cmd"
        if [ "$k" -eq 0 ]; then
            plan+="T$PANEFUL_US$cwd$PANEFUL_US$cmd$PANEFUL_US"$'\n'
        else
            plan+="P${PANEFUL_US}right$PANEFUL_US$((k - 1))$PANEFUL_US$cwd$PANEFUL_US$cmd$PANEFUL_US"$'\n'
        fi
        k=$((k + 1))
    done <<<"$resolved"
    [ "$k" -gt 0 ] || return 0
    plan+="N${PANEFUL_US}paneful"$'\n'
    created=$(term_build <<<"$plan") || return 0
    if ! terminal_has env; then
        while IFS=$PANEFUL_US read -r paneId cwd run prefill; do
            [ -n "$paneId" ] && [ -n "$run" ] && { wait_ready "$paneId" "$cwd" || true; term_type "$paneId" "$run" 1; }
        done <<<"$created"
    fi
}
...
# --- commands --------------------------------------------------------------
cmd_restore() {
    local dry=0 force=0 name=latest mode=auto style=row policy=$PANEFUL_REPLAY_POLICY
    while [ $# -gt 0 ]; do
        case $1 in
            --dry-run) dry=1; shift ;;
            --force) force=1; shift ;;
    ...
    local snap
    snap=$(snapshot_read "$name") || die "no snapshot '$name' (try: paneful list)"
    providers_load
    terminal_load
    term_launch
    ...
    already=$(awk -F"$PANEFUL_US" '$11 == "skip" && $12 ~ /^already running/' <<<"$evaluation" | grep -c . || true)
    matched=$(awk -F"$PANEFUL_US" '$11 == "type" || $11 == "prefill" || $11 == "skip"' <<<"$evaluation" | grep -c . || true)
    actionable=$(awk -F"$PANEFUL_US" '$11 == "type" || $11 == "prefill"' <<<"$evaluation" | grep -c . || true)
    ...
    if [ "$mode" = inplace ]; then
        say "mode: inplace ($term_label still has this layout; sessions go back into their panes)"
    ...
    nohup "$PANEFUL_BIN_DIR/paneful" save --force --delay 20 >/dev/null 2>&1 </dev/null &
    say "done; a fresh snapshot follows in 20s (paneful log shows this run)"
}

cmd_simulate() {
    local name=${1:-latest} snap sessions layout evaluation newtab policy=$PANEFUL_REPLAY_POLICY
    snap=$(snapshot_read "$name") || die "no snapshot '$name' (run: paneful save)"
    providers_load
    terminal_load
    ...
    newtab=$(awk -F"$PANEFUL_US" '$11 == "newtab"' <<<"$evaluation" | grep -c . || true)
    printf '\n   %s pane(s) act on their own' "$(awk -F"$PANEFUL_US" '$11 == "type" || $11 == "prefill"' <<<"$evaluation" | grep -c . || true)"
    if [ "$newtab" -gt 0 ]; then
        printf '; %s would need `paneful restore` to open elsewhere:\n' "$newtab"
        describe_decisions "$(awk -F"$PANEFUL_US" '$11 == "newtab"' <<<"$evaluation")" would
    else
        printf '; nothing left over.\n'
    fi

    printf '\nB. %s came back empty (a crash). The first shell runs `paneful restore`, which builds:\n' "$term_label"
    ...
    printf '\nNothing was changed. `paneful log` shows what real runs did.\n'
}
...
cmd_diff() {
    ...
    printf 'snapshot %s from %s vs the live %s\n\n' "$name" "$(jq -r .savedAtLocal <<<"$snap")" "$term_label"
    ...
    while IFS=$PANEFUL_US read -r provider sid name2; do
        [ -n "$sid" ] || continue
    ...
    done < <(printf '%s' "$sessions" | awk -F'\t' -v us="$PANEFUL_US" 'NF { print $1 us $3 us $5 }')
}
...
finish_if_complete() {   # $1 start epoch
    ...
    done < <(jq -r '.windows[].tabs[].terminals[].session | select(. != null) | .sessionId' "$PANEFUL_LATEST")
    [ "$covered" -ge "$total" ] || return 0
    touch "$PANEFUL_STATE_DIR/dismissed-$1"
    logfile "restore complete: all $total sessions are running or claimed; ordinary snapshots resume"
    if [ "$(jq '.unplaced | length' "$PANEFUL_LATEST")" -gt 0 ]; then
        nohup "$PANEFUL_BIN_DIR/paneful" unplaced >/dev/null 2>&1 </dev/null &
    fi
    nohup "$PANEFUL_BIN_DIR/paneful" save --force --delay 20 >/dev/null 2>&1 </dev/null &
}
...
pane_lookup() {   # $1 paneId, $2 layout -> wi ti si liveCwd snapCwd provider sid name cmd
    ...
    pos=$(awk -F'\t' -v id="$1" -v us="$PANEFUL_US" '$1 == "S" { n[$2 "," $3]++; if ($4 == id) { print $2 us $3 us n[$2 "," $3] us $5; exit } }' <<<"$2")
    [ -n "$pos" ] || return 0
    IFS=$PANEFUL_US read -r wi ti si cwd <<<"$pos"
    jq -r --argjson wi "$wi" --argjson ti "$ti" --argjson si "$si" --arg cwd "$cwd" "
        ((.windows[\$wi - 1].tabs // []) | map(select(.index == \$ti)) | .[0].terminals // [])[\$si - 1] // {}
        | [\$wi, \$ti, \$si, \$cwd, (.cwd // \"\"), (.session.provider // \"\"), (.session.sessionId // \"\"),
           (.session.name // \"\"), (.command // \"\")] | $JQ_REC" "$PANEFUL_LATEST"
}
...
cmd_first_prompt() {   # $1 tty, $2 cwd
    ...
    if ! restore_pending; then
        nohup "$PANEFUL_BIN_DIR/paneful" register-self "$tty" "$cwd" >/dev/null 2>&1 </dev/null &
        ...
        nohup "$PANEFUL_BIN_DIR/paneful" save --delay 2 --debounce 300 >/dev/null 2>&1 </dev/null &
        return 0
    fi
    providers_load
    paneId=$(cmd_register_self "$tty" "$cwd")
    if [ -z "$paneId" ]; then
        logfile "$tty: $term_label did not identify this pane; nothing resumed here"
        first_prompt_notice
        return 0
    fi
    layout=$(term_dump)
    ...
    if [ "$(grep -c $'^S\t' <<<"$layout")" -eq 1 ] && [ "$(grep -c $'^W\t' <<<"$layout")" -eq 1 ]; then
        if mkdir "$PANEFUL_STATE_DIR/rebuild-$start" 2>/dev/null; then
            logfile "$tty pane ${paneId:0:8}: $term_label has one empty pane; rebuilding the snapshot from $(jq -r .savedAtLocal "$PANEFUL_LATEST")"
            printf 'rebuild\n'
            return 0
        fi
    fi

    IFS=$PANEFUL_US read -r wi ti si lcwd scwd provider sid name cmd <<<"$(pane_lookup "$paneId" "$layout")"
    ...
    if [ -z "$scwd" ]; then
        ...
        if [ -n "$sid" ] && session_is_running "$sid" "$sessions"; then
            ...
            out=$(pane_command "$provider" "$sid" "$lcwd" "$cmd" "$PANEFUL_REPLAY_POLICY")
            mode=${out%%$'\t'*}
            if [ "$mode" = none ]; then
...
first_prompt_notice() {
    ...
    jq -r '"paneful: restoring the snapshot from \(.savedAtLocal) (\(.counts.tabs) tabs, \(.counts.sessions) sessions); each pane picks up its own as it opens. `paneful restore` fills in anything missed, `paneful dismiss` stops it."' "$PANEFUL_LATEST"
}
