...
term_label="tmux"
term_caps="env tabtitle split type"
term_verified="verified against tmux 3.7"
PANEFUL_NATIVE_TTY_MAP=1

_tm() { tmux "$@"; }

term_present() { command -v tmux >/dev/null 2>&1 && tmux has-session 2>/dev/null; }
term_current() { [ -n "${TMUX:-}" ] && term_present; }
term_version() { tmux -V 2>/dev/null | sed 's/^tmux //'; }

# tmux gained `-e` (environment for a new pane) in 3.2.
_tm_has_env() {
    local v
    v=$(term_version | sed 's/[^0-9.].*$//')
    [ -n "$v" ] || return 1
...
term_tty_pairs() {   # TSV: tty  paneId
    _tm list-panes -a -F '#{pane_tty}	#{pane_id}' 2>/dev/null |
        awk -F'\t' '{ sub("^/dev/", "", $1); print $1 "\t" $2 }'
}

term_pane_for_tty() {   # $1 tty
    term_tty_pairs | awk -F'\t' -v t="$1" '$1 == t { print $2; exit }'
}
...
_tm_env() {   # $1 run, $2 prefill
    ...
    [ -n "$1" ] && TM_ENV+=(-e "$PANEFUL_ENV_RUN=$1")
    [ -n "$2" ] && TM_ENV+=(-e "$PANEFUL_ENV_PREFILL=$2")
    return 0
}
