...
# exercised against a running WezTerm. `paneful terminals` says so.
#
...
term_present() { command -v wezterm >/dev/null 2>&1 && wezterm cli list >/dev/null 2>&1; }
term_current() { [ -n "${WEZTERM_PANE:-}" ] && term_present; }
term_version() { wezterm -V 2>/dev/null | awk '{print $2; exit}'; }
term_start_epoch() {
    local pid
...
_wz_cmd() {   # $1 run, $2 prefill -> fills WZ_CMD with the trailing "-- ..." part
    ...
    if [ -n "$1" ] || [ -n "$2" ]; then
        WZ_CMD=(-- /usr/bin/env)
        [ -n "$1" ] && WZ_CMD+=("$PANEFUL_ENV_RUN=$1")
        [ -n "$2" ] && WZ_CMD+=("$PANEFUL_ENV_PREFILL=$2")
        WZ_CMD+=("$shell" -l)
    fi
    return 0
}
