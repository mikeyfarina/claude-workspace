...
# a running kitty. `paneful terminals` says as much.
#
...
term_current() { [ -n "${KITTY_WINDOW_ID:-}" ] && term_present; }
term_version() {
...
term_tty_pairs() {   # kitty reports each pane's pid, which is better than a probe
    local pid id tty
...
term_set_tab_title() { _ky set-tab-title --match "id:$1" -- "$2"; }
...
_ky_env_args() {   # $1 run, $2 prefill -> fills KY_ENV
    KY_ENV=()
    [ -n "$1" ] && KY_ENV+=(--env "$PANEFUL_ENV_RUN=$1")
    [ -n "$2" ] && KY_ENV+=(--env "$PANEFUL_ENV_PREFILL=$2")
    return 0
}
